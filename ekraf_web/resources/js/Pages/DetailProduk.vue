          ...
          <div class="animate-in delay-100" style="display:flex;flex-direction:column;gap:1.5rem;">
            ...
            <div style="background:#ffffff;border:1px solid var(--clr-border);box-shadow:3px 3px 0px rgba(28,25,23,0.1);padding:1.5rem;border-radius:2px;">
              ...
              >
                ...
                <span>Pesan via WhatsApp {{ formattedPhoneDisplay ? `(${formattedPhoneDisplay})` : '' }}</span>
              </a>
              <a
...
const formattedPhoneDisplay = computed(() => {
  const phone = produk.value?.identitas?.no_wa || produk.value?.users?.no_hp || produk.value?.identitas?.no_hp || '';
  if (!phone) return '';
...
const waLink = computed(() => {
  const phone = produk.value?.identitas?.no_wa || produk.value?.users?.no_hp || produk.value?.identitas?.no_hp || '6281234567890';
